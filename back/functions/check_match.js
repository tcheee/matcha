...
const match = require('../functions/create_match.js')
const match = require('../functions/delete_match.js')
    ...
    if (like == 1) {
        db.query('Select * from public.likes where from_mail = $1 and to_mail = $2 and like = 1;' [body.to_mail, body.from_mail], (err, res) => {
            if (err) {
                ...
                if (res.rows[0] != undefined) {
                    const match_creation = match.create_match(from_mail, to_mail);
        ...
        db.query('Select * from public.matches where mail_a = $1 and mail_b = $2;' [body.to_mail, body.from_mail], (err, res) => {
            if (err) {
                ...
                if (res.rows[0] != undefined) {
                    const match_deletion = match.delete_match(from_mail, to_mail);
